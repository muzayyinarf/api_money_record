...
$today = new DateTime($_POST['today']); //string -> date time
$this_month = $today->format('Y-m'); //2022-06

$day7 = $today ->format('Y-m-d'); 
$day6 = date_sub($today, new DateInterval('P1D'))-> format('Y-m-d'); //date_sub-> mengurangi interval waktu
$day5 = date_sub($today, new DateInterval('P1D'))->format('Y-m-d'); //DateInterval --> Periode waktu scr spesifik
...
if($result_month -> num_rows>0){
    while($row_month=$result_month->fetch_assoc()){
        $type = $row_month['type'];// Pemasukan atau Pengeluaran
...
if($result_week->num_rows>0){
    while($row_week = $result_week ->fetch_assoc()){
        $type = $row_week["type"];// Pemasukan atau Pengeluaran
        $date = $row_week["date"];
        if($type=="Pengeluaran"){
